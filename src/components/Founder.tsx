...
export default function Founder() {
    ...
    return (
        <section className="w-full bg-[#020308] py-12 md:py-20 px-4 md:px-6 relative z-10 border-t border-white/5 flex items-center justify-center overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto w-full relative z-10">
                <div className="flex flex-col md:flex-row items-center gap-8 md:gap-20">
                    ...
                    <div className="w-full md:w-1/2 flex justify-center md:justify-end relative h-[320px] sm:h-[380px] md:h-[500px]">
                        {founders.map((founder, idx) => (
                            <div
                                key={idx}
                                className={`absolute top-0 right-auto md:right-0 w-[85vw] max-w-[320px] md:w-[500px] lg:w-[600px] h-[320px] sm:h-[380px] md:h-[500px] rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl transition-opacity duration-1000 ${activeIndex === idx ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
                            >
                                <div
                    ...
                    <div className="w-full md:w-1/2 relative flex flex-col items-center md:items-start text-center md:text-left mt-8 md:mt-0">
                        ...
                        <div className="relative w-full mb-6 md:mb-8">
                            {founders.map((founder, idx) => (
                                ...
                                >
                                    <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-heading font-black text-white tracking-tight mb-2 leading-[1.1]">{founder.name}</h2>
                                    <h3 className="text-lg md:text-2xl text-secondary font-sans mb-4 md:mb-8">{founder.role}</h3>

                                    <p className="text-[15px] md:text-lg text-text-secondary leading-relaxed mb-4 md:mb-6 font-sans">
                                        "{founder.quote}"
                                    </p>

                                    <p className="text-xs md:text-base text-white/50 leading-relaxed font-sans mb-6 md:mb-8 max-w-lg">
                                        {founder.desc}
                                    </p>

                                    <div className="flex gap-3 md:gap-4">
                                        <a href="#" className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-primary hover:text-black hover:border-primary transition-all duration-300">
                        ...
                        <div className="flex items-center gap-4 md:gap-6 mt-2 md:mt-4">
                            <div className="flex gap-2 md:gap-3">
                                <button onClick={prevSlide} className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors" aria-label="Previous leadership slide">
                                    <ChevronLeft size={16} />
                                </button>
                                <button onClick={nextSlide} className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors" aria-label="Next leadership slide">
                            ...
                            <div className="flex gap-2 md:gap-3">
                                {founders.map((_, idx) => (
                                    <button
                                        ...
                                        className={`h-1.5 rounded-full transition-all duration-500 ${activeIndex === idx ? 'w-6 md:w-8 bg-primary' : 'w-2 bg-white/20 hover:bg-white/40'}`}
